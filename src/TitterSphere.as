...
;
	...
	public class TitterSphere extends Sprite
	{
		// Environment map.
		...
		// constants
		private var sphereRadius:int = 300;
		...
		private var maxTweets:Number=500;
		
		/**
		 * Constructor
		 */
		public function TitterSphere()
		{
		...
		private function screensaverOpen() : void
		{
			...
			var _loc_1:* = 0;
			while (_loc_1 < this.meshes.length)
			{
				
				if (_loc_1 == (this.meshes.length - 1) && this.screensaverOn == true)
				{
					TweenLite.to(this.meshes[_loc_1], 2.5, {overwrite:3,ease:Bounce.easeOut,delay:(_loc_1*0.01),x:this.meshes[_loc_1].x * screenSaverScale, y:this.meshes[_loc_1].y * screenSaverScale, z:this.meshes[_loc_1].z * screenSaverScale, onComplete:this.nextPhase, onCompleteParams:["close"]});
				}
				else
				{ 
					TweenLite.to(this.meshes[_loc_1], 2.5, {overwrite:3,ease:Bounce.easeOut,delay:(_loc_1*0.01), x:this.meshes[_loc_1].x * screenSaverScale, y:this.meshes[_loc_1].y * screenSaverScale, z:this.meshes[_loc_1].z * screenSaverScale});
				}
				_loc_1++;
			}
			return;
		...
		private function screensaverCloseDo() : void
		{
			...
			while (_loc_1 >= 0)
			{
				...
				if (_loc_1 == 1 && this.screensaverOn == true)
				{
					...
					TweenLite.to(this.meshes[_loc_1], 2.5, {overwrite:3,delay:(delayCount*0.01), x:this.meshes[_loc_1].origX, y:this.meshes[_loc_1].origY, z:this.meshes[_loc_1].origZ, onComplete:this.nextPhase, onCompleteParams:["open"]});
				}
				else
				{
					TweenLite.to(this.meshes[_loc_1], 2.5, {overwrite:3,delay:(delayCount*0.01),x:this.meshes[_loc_1].origX, y:this.meshes[_loc_1].origY, z:this.meshes[_loc_1].origZ});
				}
				delayCount++;
				_loc_1--;
			}
		...
		private function setupAway3D4():void
		{
			...
			cameraController.minTiltAngle = -180
		...
		private function getPlaneGeom():PlaneGeometry {
			var thePlaneGeom:PlaneGeometry = new PlaneGeometry(14, 14, 1, 1, false,true);
			var tileScaleHeight:Number = 25 * this.scaling + Math.random() * 5;
			var tileScaleWidth:Number = tileScaleHeight;
			thePlaneGeom.height = tileScaleHeight;
			thePlaneGeom.width = tileScaleHeight;
			return thePlaneGeom;
		}
		...
		private function onRowLoader() : void
		{
			...
			loader.loadStatus.text=("count"+tweetCount.toString()+" total:"+String(twitter.tweets.length))
			var _loc_1:* = 22;
			var _loc_2:* = Math.sin(this.currentRow * (Math.PI / 2 / 12)) * this.sphereRadius;
			var _loc_3:* = Math.sqrt(this.sphereRadius * this.sphereRadius * this.scaling * this.scaling - _loc_2 * _loc_2);
			var _loc_4:* = 25 * this.scaling + Math.random() * 5;
			var _loc_5:* = 30;
			var _loc_6:* = 2 * Math.PI * _loc_3;
			var _loc_7:* = (_loc_4 + _loc_5) / _loc_6 * 360;
			var _loc_8:* = this.degrees * Math.PI / 180;
			var _loc_9:* = 1.1 + Math.random() * 0.15;
			
			
			
			var tweetMeshInstance:TweetMesh = new TweetMesh();
			
			//var tileScaleHeight:Number = 25 * this.scaling * 5;
			//var tileScaleWidth:Number = tileScaleHeight;
			//this.planeGeom.height = tileScaleHeight;
			//this.planeGeom.width = tileScaleHeight;
			
			//var cubeGeom:CubeGeometry=new CubeGeometry(tileScaleHeight,tileScaleWidth,5);
			tweetMeshInstance.init(this.twitter.tweets[this.tweetCount], planeGeom, false);
			
			
			
			//tweetMeshInstance.position.x = x;
			//tweetMeshInstance.position.y = - ( i * 4 ) + 225;
			//tweetMeshInstance.position.z = 550 * Math.cos( phi );
			
			tweetMeshInstance.addEventListener("profileLoaded", this.showMesh);
			this.degrees = this.degrees + _loc_7;
		...
		private function calcPlaneScale():void {
			var newScale:Number=Scale(meshes.length,100,500,0.5,0.3);
			trace("new scale:"+newScale);
			var tileScaleHeight:Number = 25 * this.scaling * 5;
			var tileScaleWidth:Number = tileScaleHeight;
			this.planeGeom.destHeight = tileScaleHeight;
			this.planeGeom.destWidth = tileScaleHeight;
		}
		private function moveTweets():void {
			...
			for (var i:Number=0;i<this.meshes.length;i++){
				...
				//
				TweenLite.to(tweetMeshInstance, 1, {scaleX:newScale,scaleY:newScale,overwrite:3,x:destx,y:desty,z:destz,onUpdate:onLookAtTween, onUpdateParams:[tweetMeshInstance]});
				//tweetMeshInstance.moveTo(sphereRadius * Math.cos( theta ) * Math.sin( phi ),sphereRadius * Math.sin( theta ) * Math.sin( phi ),sphereRadius * Math.cos( phi ));
		...
		private function showMesh(event:Event) : void
		{
			...
			var _loc_2:* = event.currentTarget as TweetMesh;
			_loc_2.visible = true;
		...
		private function tweetClick(param1:MouseEvent3D = null, param2:TweetMesh = null) : void
		{
			...
			if (param1 != null)
			{
				...
				TweenLite.to(currentMesh, 0.3, {x:currentMesh.x * 1.15, y:currentMesh.y * 1.15, z:currentMesh.z * 1.15, onComplete:this.onFinishClickTween, onCompleteParams:[currentMesh]});
			}
			else if (param2 != null)
			{
			...
			TweenLite.to(tweetWinInstance, 0.8, {alpha:1, x:_loc_10.x, y:_loc_10.y, z:0, rotationX:0, rotationY:0, rotationZ:0, width:_loc_4.width, height:_loc_4.height});
			currentMesh = null;
			_loc_4 = null;
			_loc_5 = null;
		...
		{
			TweenLite.to(param1, 0.4, {x:param1.x / 1.15, y:param1.y / 1.15, z:param1.z / 1.15});
			return;
		}
		private function closeWindow(event:Event = null, param2:TweetWindow = null, param3:String = "off") : void
		{
			...
			TweenLite.to(_loc_4, 0.4, {alpha:0.2, x:_loc_5.x, y:_loc_5.y, z:_loc_5.z, width:_loc_4.tweetRef.planeSize, height:_loc_4.tweetRef.planeSize, onComplete:this.destroyWindow, onCompleteParams:[_loc_4]});
		...
		private function displayNewTweet(event:Event) : void
		{
			...
			_loc_3.init(this.twitter.tweets[0], this.planeGeom, false);
			_loc_3.x = _loc_2.x;
			_loc_3.y = _loc_2.y;
			_loc_3.z = _loc_2.z;
			...
			TweenLite.to(_loc_2, 1, {scaleX:0,scaleY:0,scaleZ:0, onComplete:killTweetMesh, onCompleteParams:[_loc_2]});
			
			} else {
				trace("push mesh");
				var newMesh:TweetMesh = new TweetMesh();
				newMesh.init(this.twitter.tweets[0], this.planeGeom, false);
		...
		private function killTweetMesh(theMesh:TweetMesh):void {
			scene.removeChild(theMesh);
			theMesh.kill();
			theMesh.disposeAsset();
			theMesh.dispose();
			theMesh=null;
		}
		...
		private function setupBall() : void
		{
			...
			TweenLite.to(this.loader, 0.3, {alpha:0});
		...
		{
			// plane geometry???
			//planeGeom = new PlaneGeometry(14, 14, 1, 1, false);
			planeGeom = new CustomCubeGeometry(14, 14, 3);
